...
function ItemModal({ onClose, selectedCard, onDeleteItem, isLoggedIn }) {
  const currentUser = React.useContext(CurrentUserContext);
  const isOwn = selectedCard.owner === currentUser._id;
  const itemDeleteButtonClassName = `modal__delete-button ${isOwn && isLoggedIn ? "modal__delete-button_visible" : "modal__delete-button_hidden"}`;
